...
var serialize = function(root) {
    ...
    while (nextRow.length) {
        ...
        for (var i=0; i<currentRow.length; i++) {
            ...
            if (p) {
                nextRow.push(p.left);
                nextRow.push(p.right);
            }
            result.push(p ? p.val : "null");
        }
    }
...
var deserialize = function(data) {
    ...
    while (i<len && j<len) {
        ...
        var right = (j+1<len && a[j+1]!="null") ? new TreeNode(Number(a[j+1])): null;
        b.push(left, right);
        b[i].left = left;
        b[i].right = right;
        i ++;
...
var root = myBinaryTree.arrayToTree([1,2,null,4,5]);
var s = serialize(root);
var r2 = deserialize(s);
console.log(s);
console.log(JSON.stringify(r2, null, 2));

root = deserialize("[1,2,3,null,null,4]");
s = serialize(root);
console.log(s);
