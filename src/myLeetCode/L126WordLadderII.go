...
func main() {
	...
	res = findLadders("hot", "dog", []string{"hot", "dog"})
	fmt.Printf("%v\n", res)

	res = findLadders("cet", "ism",
...
func findLadders(beginWord string, endWord string, wordList []string) [][]string {
	res := make([][]string, 0)
	dict := StringList2BoolMap(wordList)
	...
	for len(toVisit) > 0 && !end {
		...
		for i := 0; i < l; i++ {
			...
			for j := range curRune {
				...
				for k := byte(0); k < 26; k++ {
					curRune[j] = rune('a' + k)
					newWord := string(curRune)
					if newWord == curWord || !dict[newWord] {
						continue
					}
					...
					if newWord == endWord {
						...
						path := appendWordToPath(paths[i], endWord)
						res = append(res, path)
...
func appendWordToPath(paths []string, word string) []string {
